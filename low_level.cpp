...
private:
  ...
  solveLinearSystem(const std::vector<std::vector<double>> &a,
...
int main() {
  ...
  if (r.status_code == 200) {
    ...
    std::cout << "Processing task " << task.identifier << " of size "
              << task.size << std::endl;
    task.work(); // Résoudre Ax = b

    // POST Request Example
    cpr::Response postResponse = cpr::Post(
        cpr::Url{"http://localhost:8000"}, cpr::Body{task.to_json().dump()},
    ...
    if (postResponse.status_code == 200) {
      ...
      std::cerr << "Failed to send Task data: " << postResponse.error.message
                << std::endl;
    }
  } else {
